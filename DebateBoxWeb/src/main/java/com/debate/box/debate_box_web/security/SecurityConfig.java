...
class customSuccessHandler implements AuthenticationSuccessHandler {
    ...
    // private void handleRedirect(HttpServletRequest request, HttpServletResponse
    // response,
    ...
    // .anyMatch(authority ->
    // authority.getAuthority().equals("ROLE_HEALTHCARE_PROFESSIONAL"));

    // boolean isInventoryManager = authorities.stream()
    // .anyMatch(authority ->
    // authority.getAuthority().equals("ROLE_INVENTORY_MANAGER"));

    // boolean isLabTechnician = authorities.stream()
    // .anyMatch(authority ->
    // authority.getAuthority().equals("ROLE_LAB_TECHNICIAN"));
